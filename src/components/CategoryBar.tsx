...
export default function CategoryBar({
  ...
  useEffect(() => {
    ...
    // Reset liquid stretch scaling after transition settles
    ...
    <div id="categories" className="w-full py-6">
      ...
      >
        <div className="relative flex items-center gap-2 min-w-max py-1">
          {/* Animated Liquid/Morphing Active Indicator Background */}
          <div
            className="absolute bg-[#090d12] shadow-lg shadow-black/25 pointer-events-none will-change-transform"
            style={{
          ...
          {CATEGORIES.map((category) => {
            ...
            return (
              <button
                ...
                className={`relative z-10 whitespace-nowrap px-4 py-2 text-xs font-semibold rounded-xl transition-colors duration-200 ${
                  isActive
                    ? 'text-[#F1FEC8] font-bold'
                    : 'text-[#090d12]/80 hover:text-[#090d12] hover:bg-slate-900/10 border border-slate-900/15'
                }`}
              >
                {category}
              </button>
